...
type Props = {
  status: string;
  handleFullView: () => void;
  handleDetailedScrren: () => void;
};

const AppointmentDetails = ({
  status,
  handleFullView,
  handleDetailedScrren,
}: Props) => {
  const completed =
    status === 'upcomming' || status === 'inprogress' || status === 'completed';
  return (
    <Flex overrideStyle={{position: 'relative'}}>
      <Flex>
        ...
        <PrescriptionList handleDetailedScrren={handleDetailedScrren} />
      </Flex>
      <Text type="heading500" overrideStyle={styles.detailsText}>
        Appointment Details :
      </Text>
